...
handType = \cards ->
    isJoker = \card -> card == J
    amountOfJokers = List.countIf cards isJoker
    cardGroups = count (List.dropIf cards isJoker)
    differentCards = Dict.len cardGroups
    groupSizes = Dict.values cardGroups |> List.sortDesc
    groupSizesWithJokers = List.update groupSizes 0 \highestFigure -> highestFigure + amountOfJokers
    when (differentCards, groupSizesWithJokers) is
        (1, _) -> Recamel
        (2, [4, 1]) -> Camel
        (2, _) -> Tabernacle
        (3, [3, 1, 1]) -> Triplet
        (3, _) -> DoublePair
        (4, _) -> Pair
        _ -> if amountOfJokers == 5 then Recamel else HighCard

expect handType [N 3, N 2, T, N 3, K] == Pair
expect handType [T, N 5, N 5, J, N 5] == Camel
expect handType [K, K, N 6, N 7, N 7] == DoublePair
expect handType [K, T, J, J, T] == Camel
expect handType [Q, Q, Q, J, A] == Camel
expect handType [A, A, A, A, A] == Recamel
expect handType [J, J, J, J, N 4] == Recamel
expect handType [N 2, N 3, N 5, T, J] == Pair
expect handType [N 2, N 3, N 5, T, Q] == HighCard
expect handType [K, K, K, Q, Q] == Tabernacle
expect handType [K, K, K, Q, T] == Triplet
expect handType [K, K, Q, Q, T] == DoublePair
expect handType [J, J, J, J, J] == Recamel
